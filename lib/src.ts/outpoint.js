...
function encodeOutpointInput(txid, pos) {
    const input = {
        txid: safe_buffer_1.Buffer.from(txid, "hex"),
        vout: pos,
    };
    const str = safe_buffer_1.Buffer.from(metashrew_runes_1.Outpoint.toBinary(input)).toString("hex");
    return "0x" + str;
}
function decodeRunes(balances) {
    ...
    return balances.entries.map((entry) => {
        const balance = safe_buffer_1.Buffer.from(entry.balance);
        const d = entry.rune;
        const spacer = "•";
        const bitField = d.spacers.toString(2);
        let name = safe_buffer_1.Buffer.from(d.name).toString("utf-8");
        ...
        return {
            rune,
            balance: BigInt("0x" + safe_buffer_1.Buffer.from(balance).toString("hex")),
        };
    });
}
function decodeOutpointViewBase(op) {
    return {
        ...
        outpoint: {
            txid: safe_buffer_1.Buffer.from(op.outpoint.txid).toString("hex"),
            vout: op.outpoint.vout,
        },
        output: op.output
            ? {
                value: op.output.value,
                script: safe_buffer_1.Buffer.from(op.output.script).toString("hex")
            }
            : { value: "", script: "" },
        height: op.height,
        txindex: op.txindex,
...
function decodeOutpointView(hex) {
    const bytes = Uint8Array.from(safe_buffer_1.Buffer.from((0, utils_1.stripHexPrefix)(hex), "hex"));
