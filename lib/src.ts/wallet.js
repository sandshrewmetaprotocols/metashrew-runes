...
function decodeWalletOutput(hex) {
    const wo = metashrew_runes_1.WalletResponse.fromBinary(Uint8Array.from(node_buffer_1.Buffer.from((0, utils_1.stripHexPrefix)(hex), "hex")));
    return {
...
function encodeWalletInput(address) {
    const input = {
        wallet: Uint8Array.from(node_buffer_1.Buffer.from(address, "utf-8")),
    };
    return "0x" + node_buffer_1.Buffer.from(metashrew_runes_1.WalletRequest.toBinary(input)).toString("hex");
}
