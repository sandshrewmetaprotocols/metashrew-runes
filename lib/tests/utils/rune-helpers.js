...
const initCompleteBlockWithRuneEtching = (outputs, pointer, divisibility = 8, premineAmount = 2100000005000000n, runeName = "GENESIS•RUNE•FR", symbol = "G", block, skipTx = 0) => {
    let coinbase;
    if (block == undefined) {
    ...
    Array.from(new Array(skipTx)).map((d, i) => {
        prevTx = (0, block_helpers_1.buildTransaction)([
            {
                hash: prevTx.getHash(),
                index: i,
